...
foreach (array('get_int', 'post_int', 'get_post_int', 'post_get_int', 'get_time', 'post_time', 'get_post_time', 'post_get_time', 'get_date', 'post_date', 'get_post_date', 'post_get_date', 'get_datetime', 'post_datetime', 'get_post_datetime', 'post_get_datetime') as $func) {
    if (!function_exists($func)) {
...
class CI_DB_sqlite3_result extends CI_DB_result
{
    ...
    function _fetch_assoc()
    {
        if (is_array($this->pdo_results)) {
            $i = $this->pdo_index;
            $this->pdo_index++;
            if (isset($this->pdo_results[$i])) return $this->pdo_results[$i];
            return null;
        }
        return $this->result_id->fetch(PDO::FETCH_ASSOC);
    }
...
class phpfastcache_sqlite extends phpFastCache implements phpfastcache_driver
{
    ...
    function indexing($keyword)
    {
        ...
        if (isset($row['db']) && $row['db'] != "") {
            ...
            $stm->execute(array(":keyword" => $keyword, ":db" => $db,));
        }
        return $db;
    }
    ...
    function driver_set($keyword, $value = "", $time = 300, $option = array())
    {
        ...
        if ($toWrite == true) {
            try {
                ...
                $stm->execute(array(":keyword" => $keyword, ":object" => $this->encode($value), ":exp" => @date("U") + (Int)$time,));
                return true;
            } catch (PDOException $e) {
    ...
    function deleteRow($row)
    {
        ...
        $stm->execute(array(":id" => $row['id'], ":U" => @date("U"),));
    }

    function driver_delete($keyword, $option = array())
    {
        $stm = $this->db($keyword)->prepare("DELETE FROM `caching` WHERE (`keyword`=:keyword) OR (`exp` <= :U)");
        $stm->execute(array(":keyword" => $keyword, ":U" => @date("U"),));
    }
...
class phpFastCache
{
    ...
    public static $config = array("storage" => "auto", "fallback" => array(), "securityKey" => "", "htaccess" => false, "path" => "", "server" => array(array("127.0.0.1", 11211, 1),), "extensions" => array(),);
    var $tmp = array();
    var $checked = array("path" => false, "fallback" => false, "hook" => false,);
    var $is_driver = false;
    var $driver = NULL;
    var $option = array("path" => "", "htaccess" => null, "securityKey" => '', "system" => array(), "storage" => "", "cachePath" => "",);
    ...
    {
        $object = array("value" => $value, "write_time" => @date("U"), "expired_in" => $time, "expired_time" => @date("U") + (Int)$time,);
